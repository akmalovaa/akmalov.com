...
Руководство по работе с Claude Code — AI-ассистентом для разработки от Anthropic. Работает в терминале, VS Code и JetBrains.
...
| **Sonnet** | Быстрая и качественная (4.6 уже очень хороша) | Ежедневная работа, большинство задач |
...
`Shift+Tab` переключает режимы: **default** → **acceptEdits** → **plan**
...
1. Claude **только читает** файлы и анализирует код
2. Пишет **детальный план** в markdown
3. Вы **проверяете** и даёте фидбек
4. После одобрения — выполнение
...
Включение: `/plan`, `Shift+Tab` или флаг `--permission-mode plan`
...
| **plan** | Только чтение, планирование |
...
- **Скриншоты** — можно отправлять изображения для анализа (`Ctrl+V`)
- **Фоновые задачи** — `/tasks` для управления длительными операциями
- **Голосовой ввод** — `/voice` (push-to-talk, 20 языков)
- **Удалённый доступ** — `/remote-control` продолжить сессию с телефона
- **Экспорт** — `/export` сохранить разговор в файл
- **Пайпы** — `cat error.log | claude -p "объясни"` — подать данные на вход
...
- `/cost` — токены и стоимость (API)
- `/stats` — паттерны использования (подписка)
- `/usage` — лимиты тарифа

### Как экономить токены

1. **Контекст**: `/clear` между задачами, `/compact` с фокусом на важное
2. **Модель**: Sonnet для большинства задач, Opus для сложных, Haiku для субагентов
3. **MCP**: Отключайте неиспользуемые серверы (`/mcp`). Инструменты загружаются отложенно
4. **CLAUDE.md**: До 200 строк. Детали — в скилы (грузятся по требованию)
5. **Субагенты**: Тяжёлые операции (тесты, логи) в субагенте — в основной контекст возвращается только результат
6. **Промты**: Конкретные запросы = меньше сканирования файлов
7. **Планирование**: Сначала plan mode, потом реализация — экономит на переделках
